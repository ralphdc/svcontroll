...
class CommonbakController extends Controller {
  ...
  /*  public function _delete($dwz_db_name = '') {
    ...
    */
     ...
     * @param string $forwordurl
     */
    //dwz封装后的返回函数
    protected function dwzReturn($merge=null,$code=200,$msg='',$navTab='',$rel='',$callBackType='',$forwordurl=''){
    	$return['statusCode'] = $code;
    	$return['message'] = $msg;
    	$return['navTabId'] = $navTab;
    	$return['rel']= $rel;
    	$return['callbackType'] = $callBackType;
    	$return['forwardUrl']=$forwordurl;
    	if($merge){
    ...
    //aReturn返回成功执行
    protected function aReturn($merge=null,$status=true,$msg=''){
    ...
    protected function page(){
    	//获取排序字段
    	$orderField = I('request.orderField');
    	//排序方式
    	$order = I('request.orderDirection');
    	//单页数量
    	$numPerPage = I('request.numPerPage');
    	//页数
    	...
    	$numPerPage = $numPerPage?$numPerPage:20;
    	...
    	$this->order = array($orderField=>$order);
    	$this->numPerPage = $numPerPage;
    	$this->pageNum = $pageNum;
    
    	$re['order'][$orderField] = $order;
    	$re['limit'] = "{$offset},{$numPerPage}";
    	return $re;
    }
