...
class DeviceTypeController extends Controller{
	...
	public function index() {
		...
		if($_GET['pageNum'])
		{
		    $_REQUEST['numPerPage'] = empty($_REQUEST['numPerPage']) ?  ($_COOKIE['deviceTypePerPage']? $_COOKIE['deviceTypePerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
		    $_REQUEST[C('VAR_PAGE')] = $_GET['pageNum'];
		    $_POST['pageNum'] = $_GET['pageNum'];
		}
		$pageNum =empty($_REQUEST['numPerPage']) ?  ($_COOKIE['deviceTypePerPage']? $_COOKIE['deviceTypePerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
	...
	function icon()
    {
        ...
        foreach ($sResult as $s){
            $imgArr = explode(".",$s);
            if($imgArr[0] != 'default' && $imgArr[0] != 'node2' && $imgArr[0] != 'childtp' && $imgArr[0] != 'enterdesk' ){
   ...
   function delicon()
   {
       ...
       if($icon){
           ...
           $post= array('iconUrl'=>$icon);
           $model = new DeviceTypeModel();
           $check = $model->checkDelete($post);
           if(isset($check['errorCode'])){
               if($check['errorCode'] == 0){
                   ...
                   $ret = array("statusCode"=>"0","message"=> '图片已经和设备绑定，请先解除关系！',"navTabId"=>"",
                       "rel"=>"","forwardUrl"=>"","callbackType"=>"");
               }
           }else{
