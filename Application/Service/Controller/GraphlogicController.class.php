...
class GraphlogicController extends CommonController{
    ...
    public function __init()
    {
        ...
        //保存当前操作类型；
        $this->action_name = "topo:lg:action:".$this->sid;
        //保存当前编辑拓扑图ID;
        $this->edit_id = "topo:lg:edit:id:".$this->sid;
    }
    ...
    public function index()
    {
         if($_GET['pageNum'])
        {
            $_REQUEST['numPerPage'] = empty($_REQUEST['numPerPage']) ?  ($_COOKIE['topolgnumPerPage']? $_COOKIE['topolgnumPerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
            $_REQUEST[C('VAR_PAGE')] = $_GET['pageNum'];
            $_POST['pageNum'] = $_GET['pageNum'];
        }
        $pageNum =empty($_REQUEST['numPerPage']) ?  ($_COOKIE['topolgnumPerPage']? $_COOKIE['topolgnumPerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
    ...
    public function editChildTP()
    {
        ...
        if($_GET['pageNum'])
        {
            $_REQUEST['numPerPage'] = empty($_REQUEST['numPerPage']) ?  ($_COOKIE['topolgChildnumPerPage']? $_COOKIE['topolgChildnumPerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
            $_REQUEST[C('VAR_PAGE')] = $_GET['pageNum'];
            $_POST['pageNum'] = $_GET['pageNum'];
        }
        
        $pageNum =empty($_REQUEST['numPerPage']) ?  ($_COOKIE['topolgChildnumPerPage']? $_COOKIE['topolgChildnumPerPage']:C('PAGE_LISTROWS') ): $_REQUEST['numPerPage'];
        $this->assign ( 'numPerPage', $pageNum ); //每页显示多少条
        $this->assign ( 'currentPage', !empty($_REQUEST[C('VAR_PAGE')])?$_REQUEST[C('VAR_PAGE')]:1);
        
        $dataModel = new GraphlogicModel();
        $result = $dataModel->findByPostForLogicChildTP($_POST,$_GET);
        $count = $dataModel->countByPost($_POST,$_GET);
        $this->assign ( 'totalCount', $count );
        $this->assign('list',$result);
