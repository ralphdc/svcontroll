<?php
/**
 * 服务管服务和java通信公共数据接口类子类（地域管理）
 * @author zengguangqiu
 *
 */
class OperatesystemModel extends CommunicationModel {
	var $server = '/omp/asset';
	var $method = array(
			'index'=>'asset.operatesystem.search',
			'add'=>'asset.operatesystem.create',
			'update'=>'asset.operatesystem.update',
			'delete'=>'asset.operatesystem.delete',
			'deleteall'=>'asset.operatesystem.deletebatch',
			'getrow'=>'asset.operatesystem.get',
			'getall'=>'asset.operatesystem.list',
	);
	...
	function findByPost ($post, $get) {
		...
		$post['data']['system']=!empty($post['system'])? $post['system']:null;
		$post['data']['remark']=!empty($post['remark'])? $post['remark']:null;
