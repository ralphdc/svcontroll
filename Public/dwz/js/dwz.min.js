var DWZ = {
    ...
    $.fn.extend({
        ...
        loadUrl: function(url, data, callback,flush) {
            $(this).ajaxUrl({
                url: url,
                data: data,
                callback: callback,
...
function Map() {
    ...
    $.extend($.fn, {
        ...
        _check: function(op) {
            ...
            ckbox.append("<input type='checkbox' style='display:none;' " + attrs + "/>").click(function() {
                ...
                $(node)._checkParent();
				// 2014-08-04 补充
				if(op.options.clickEven && typeof(op.options.clickEven)=="function"){
					op.options.clickEven($(this).parents('.tree'),$(this));
				}
    ...
    function fillSpace(key) {
        ...
        $(".accordionContent", obj).height(height+1);
    }
...
function initLayout() {
	...
	$("#container .tabsPageContent").height(iContentH - 33).find("[layoutH]").layoutH();
    $("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH);
	$('#sidebar_s .collapse').height(iContentH-2);
...
var navTab = {
    ...
    _switchTab: function(iTabIndex) {
        ...
        this._scrollCurrent();
        this._reload($tab);
        this._setCurrentMenu(iTabIndex); //2016-04-06 切换当前菜单ID,写入Session;
    },
    //2016-04-06 切换当前菜单ID,写入Session;
    _setCurrentMenu: function(iTabIndex){
    	var Current_tab = this._getTabs().eq(iTabIndex);
    	var tabId = $(Current_tab).attr("tabid");
    	var url = setMenuUrl;
	   $.ajax({
	       type:'POST',
		   url: url,
	       ...
	       success: function(data){
	    	   if(data.msg != 'sus'){
	    		   alertMsg.error('连接超时，切换失败，请刷新页面！');
	    	   }
	       },
	       error: DWZ.ajaxError
	   });
...
//add by eaglezhao 2014.8.8
function _getOtherPagerForm($parent, args) {
...
function dwzPageBreak(options) {
    ...
    if (op.rel) {
	  ...
	  // var otherForm = "#"+op.rel;
	   var form = _getOtherPagerForm($box, op.data);
        if (form) {
            $box.ajaxUrl({
                type: "POST",
                url: $(form).attr("action"),
    ...
    $.extend({
        bringBackSuggest: function(args) {
            ...
            $box.find(":input").each(function() {
                ...
                var remainValues = $input.val();
                for (var key in args) {
                    var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
                    //保留原值，修改源码； 2015-11-23
                    if (name == inputName) {
                    	if(args.remainValue == 'checked'){
                    		$input.val(remainValues+','+args[key]);
                    	}else{
                    		$input.val(args[key]);
                    	}
                        break;
        ...
        multLookup: function() {
            return this.each(function() {
                ...
                $this.click(function(event) {
                    ...
                    var $remain_bool = $unitBox.find("input[name='remainvalue']").attr('checked') || ''; //保留原值，修改源码； 2015-11-23
                    ...
                    args.remainValue = $remain_bool; //保留原值，修改源码； 2015-11-23
                    $.bringBack(args);
                });
            });
        },
