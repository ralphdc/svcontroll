...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() : 34) - 1;
...
function initUI(_box){
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			var options = {};
			var w = eval($this.attr("width") || 900);
			var h = eval($this.attr("height") || 420);
    ...
    if ($.fn.treetable)
        $(".treetable").each(function () {
            $(this).treetable({ expandable: true, column: 1 });
        }).find("tbody").on("mousedown", "tr", function () {
            $(".treetable .selected").not(this).removeClass("selected");
    ...
    $(".panelBar .toolBar .expand").bind("click", function () {
        $('.treetable').treetable('expandAll');
        return false;
    });
    $(".panelBar .toolBar .collapse").bind("click", function () {
    ...
    if ($.fn.groupTable)
        $(".grouptable").each(function () {
            var $tab = $(this);
            var groupby = $tab.attr("groupby") || 0;
            var collapse = $tab.attr("collapse") || 1;
            try {
                $tab.groupTable({ groupby: groupby, collapse: collapse });
            } catch(e) {
            } 
        }).find("tbody").on("mousedown", "tr", function () {
            if ($(this).find(".minus").length == 0) {
                $(".grouptable .selected").not(this).removeClass("selected");
                $(this).addClass("selected");
            }
        }).find("tr").click(function () {
            var $tr = $(this);
	...
	// name为ids的单选框单击是触发事件checkboxInvert。也就是单击无效果。代码在dwz.checkbox.js
	if ($.fn.checkboxInvert) $(":checkbox[name='ids']", $p).checkboxInvert($p);
	if ($.fn.multSelect) $("a[btnSelect=mult]", $p).multSelect();
	if ($.fn.singleSelect) $("a[btnSelect=single]", $p).singleSelect();
}
