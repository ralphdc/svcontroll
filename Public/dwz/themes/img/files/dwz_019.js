	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				var maxNum = $table.attr('maxNum') || 10;
				...
				if (addButTxt) {
					...
					$addBut.click(function(){
						...
						var nmSun = rowNum + $table.find('tbody tr').length;
						if( nmSun > parseInt(maxNum))
						{
							alertMsg.warn('最多不超过'+maxNum+'条');
							return;
						}
				...
				switch(field.type){
					...
					case 'lookup':
						...
						if (field.suggestFields) {
							suggestFrag = 'autocomplete="off" lookupGroup="'+field.lookupGroup+'"'+suffixFrag+' suggestUrl="'+field.suggestUrl+'" suggestFields="'+field.suggestFields+'"' + ' postField="'+field.postField+'"'+attrFrag;
						}
					...
					case 'enum':
						...
						break;
					case 'date':
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" size="'+field.size+attrFrag+'"/>'
		...
		multSelect: function() {
			function multBring(args) {
				var $box = _lookup['$target'].parents(".unitBox:first");
				$box.find(":input[name^='"+_lookup.currentGroup+".']").each(function(){
					var $input = $(this), inputName = $input.attr("name");
					var inputVal = $input.val();
					for (var key in args) {
						var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
						if (name == inputName) {
							if (inputVal.indexOf(args[key]) == -1) {
								inputVal += (inputVal=="") ? args[key] : ","+args[key];
							}
							else {
								inputVal = inputVal.replace(RegExp(",?"+args[key]+",?"), function($1){
								reg = /^,.*,$/g;
								if( !reg.test($1) )
									return "";
								else
			...
			return this.each(function(){
				var $this = $(this), args={};
				$this.click(function(event){
					var $unitBox = $this.parents("tr:first");
					var $input = $unitBox.find(":hidden[name='"+$this.attr("multLookup")+"']");
					var args = DWZ.jsonEval($input.val());
					
					if ($.isEmptyObject(args)) {
						alertMsg.error("抱歉!程序有错误,请联系开发人员.");
						return false;
					}
					multBring(args);
					if ( $this.hasClass("btnSelect") ) {
							$this.removeClass("btnSelect").addClass("btnUnSelect");
					}
					else {
							$this.removeClass("btnUnSelect").addClass("btnSelect");
					}
				});
			});
		},
