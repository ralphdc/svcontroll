...
function proSearch(domid)
{
	...
	if(doms_val.length > 0){
		$.post('/index.php/Service/Graphlogic/proSearch',{'pdName':doms_val},function(res){
			var data = eval("(" + res +")");
...
function serviceSearch(domid)
{
	...
	if(!checkEmpty(doms_val)){
		$.post('/index.php/Service/Graphlogic/serviceSearch',{'sName':doms_val},function(res){
...
function toggleService(domid)
{
	...
	$.post('/index.php/Service/Graphlogic/queryProService',{'pdName':ov},function(res){
		var datas = eval("(" + res + ")");
