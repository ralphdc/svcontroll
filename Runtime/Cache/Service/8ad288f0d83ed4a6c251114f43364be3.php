...
<form method="post" action="/index.php/Service/Servermanage/edit" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone)" novalidate="novalidate">
	<div class="pageFormContent" layouth="58" style="height: 297px; overflow: auto;">
			...
			<p>
				...
				<?php if(is_array($environment) && !empty($environment)){ foreach ($environment as $enkey=>$enval) { if($enkey == $row['environment']) $selected = 'selected'; echo '<option value='.$enkey.'  '.$selected.'>'.$enval.'</option>'; $selected = ''; } }?>
				</select>
			</p>
			<p>
				<label>机器类型：</label>
			    <select name="isVirtual" onchange="showVirtual(this.value);" style="width:100px">
				<?php if(is_array($virtual) && !empty($virtual)){ foreach ($virtual as $tkey=>$tval) { if($tkey == $row['isVirtual']) $selected = 'selected'; echo '<option value='.$tkey.'  '.$selected.'>'.$tval.'</option>'; $selected = ''; } }?>
				</select>
			</p>
			<p>
				<label>设备类型：</label>
				...
				<?php if(is_array($deviceTypeList) && !empty($deviceTypeList)){ foreach ($deviceTypeList as $tkey=>$tval) { $select = $row['devicetypeid'] == $tval['deviceid'] ? "selected='selected'" : ""; echo "<option value=".$tval['deviceid']." $select>".$tval['deviceName']."</option>"; } }?>
				</select>
			</p>
			...
			<p id="serverxh">
				...
				<?php if(is_array($MechinetypeInfo) && !empty($MechinetypeInfo)){ foreach ($MechinetypeInfo as $key=>$val) { if($val['stId'] == $row['stId']) $selected = 'selected'; echo '<option value='.$val['stId'].' '.$selected.'>'.$val['stChar'].'</option>'; $selected = ''; } }?>
				</select>
			...
			<p>
				...
				<?php if(is_array($AreamanageInfo) && !empty($AreamanageInfo)){ foreach ($AreamanageInfo as $key=>$val) { if($val['arId'] == $row['arId']) $selected = 'selected'; echo '<option value='.$val['arId'].' '.$selected.'>'.$val['arName'].'</option>'; $selected = ''; } }?>
				</select>
			...
			<p id="systemoper">
				...
				<?php if(is_array($OperatesystemInfo) && count($OperatesystemInfo)){ foreach ($OperatesystemInfo as $key=>$val) { if($val['sysid'] == $row['sysid']) $selected = 'selected'; echo '<option value='.$val['sysid'].' '.$selected.'>'.$val['system'].'</option>'; $selected = ''; } }?>
				</select>
...
<script>
	...
	function showCabit(val)
	{
		...
		$.post('/index.php/Service/Servermanage/getCabitelist',{roomid:val},function(data){
			...
			var cabinet_idval = <?php echo ($row['cabinet_id']); ?>;
