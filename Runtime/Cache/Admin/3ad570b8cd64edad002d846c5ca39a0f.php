...
<form method="post" action="/index.php?s=/Admin/WeightService/update" class="pageForm required-validate" onsubmit="return validateCallback(this, editDialogAjaxDone)" novalidate="novalidate">
	<div class="pageFormContent" layouth="58" style="height: 297px; overflow: auto;">
			<p>
				...
				<input type="hidden" size="28" name="treessname" readonly="true" value="<?php echo ($_REQUEST['treessname']); ?>">
			</p>
			<p>
				<label>IP地址：</label>
				<input type="text" size="28" name="ip" class="textInput readonly" readonly="true" value="<?php echo ($_REQUEST['ip']); ?>">
			</p>
			<p>
				<label>路径：</label>
			...
			<p>
				<label>端口：</label>
				<input type="text" size="28" name="port" class="textInput " readonly="true" value="<?php echo ($_REQUEST['port']); ?>">
			</p>
			<?php if(($_REQUEST['flag'] != 1)): ?><p>
				<label>权重：</label>
				<input type="text" size="28" name="weight" class="textInput digits" min='0' max="100" value="<?php echo ($_REQUEST['weight']); ?>" />
			</p>
			<p>
 ...
 $_REQUEST['consumergroupid'] = ($_REQUEST['consumergroupid'] == '默认组' || $_REQUEST['consumergroupid'] == '无' )?'':$_REQUEST['consumergroupid']; $_REQUEST['providergrouid'] = ($_REQUEST['providergrouid'] == '默认组' || $_REQUEST['providergrouid'] == '无' )?'':$_REQUEST['providergrouid']; ?>
				<?php if(($_REQUEST['flag'] == 1)): ?><input type="text" size="28"  maxlength="30" class="textInput"  readonly="true" value="<?php echo ($_REQUEST['consumergroupid']); ?>" name="prt.groupName" > 
				...
				<input type="text" size="28"  maxlength="30" class="textInput" readonly="true"  value="<?php echo ($_REQUEST['providergrouid']); ?>" name="prt.groupName" />
				<a class="btnLook" href="/index.php/Admin/WeightService/groupSelect" mask="false" lookupGroup="prt" width="700" height="400">查找：</a>
				<?php else: ?>
				<input type="text" size="28"  maxlength="30" class="textInput" readonly="true" value="<?php echo ($_REQUEST['providergrouid']); ?>" name="prt.groupName" />
