...
class PublicController extends Controller {
    ...
    public function checkLogin() {
        ...
        $header = C('loginIn');
        $request = array("userNo"=>$_POST['account'],"passwd"=>$passwd,"productId"=>"100","ip"=>$ip);
         
        $SendMessage= new SendLogin;
        $result =$SendMessage ->send($header,$request); */

        //print_r($result);exit;
        if($_POST['account'] == 'admin' && $_POST['password'] == '123456' )
        {
        	cookie('LoginAuth','admin');
      ...
      /*   $code = $result['code'];
        if($code != 0){
        	...
        	$_SESSION[C('USER_AUTH_KEY')]	=	$result['loginUser']['userNo'];
        	$_SESSION['login_count']	=	$result['loginUser']['xgdSessionId'];
        	$_SESSION['xgdSessionId']	=	$result['loginUser']['xgdSessionId'];
        	// 缓存访问权限
        	RBAC::saveAccessList();
        	$this->success('登录成功！',__MODULE__.'/Index/index');
        } */
