<?php
/**
 * 服务管服务和java通信公共数据接口类子类（代理资源映射）
 * @author zengguangqiu
 *
 */
class AgentresourceModel extends CommunicationModel {
	var $method = array(
			'index'=>'monitor.resref.search',
			'add'=>'monitor.resref.create',
			'update'=>'monitor.resref.update',
			'delete'=>'monitor.resref.deletebatch',
			'deleteall'=>'monitor.resref.deletebatch',
			'getrow'=>'monitor.resref.get',
			'find'=>'monitor.mount.layer.search',
	);
	...
	function findByPost ($post, $get) {
		...
		$post['data']['agentIp'] = $post['agentIp'];
		$post['data']['resIp'] = $post['resIp'];
