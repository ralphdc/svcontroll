...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			tbody.wrap("<div class='gridScroller'" + layoutStr + " style='width:100%;'><div class='gridTbody'><table style='width:" + (tlength - 20) + "px;'></table></div></div>");
			...
			$trs.hoverClass().each(function(){
				...
				// 判断是否被选中
				if ($table.attr("targetType") == 'dialog') {
					var _lookup = $.getLookup();
					var $box = _lookup['$target'].parents(".unitBox:first");
					var $input = $box.find("input[name='"+_lookup.currentGroup+"."+_lookup.pk+"']").filter(":first");
					var inputVal = $input.val();
					var chked = (new RegExp("(^"+$tr.attr("rel")+"$)|(^,"+$tr.attr("rel")+"$)|(^"+$tr.attr("rel")+",$)").test(inputVal))
					if(chked) {
							$tr.find("a").removeClass("btnUnSelect").addClass("btnSelect");
					}
					else {
							$tr.find("a").removeClass("btnSelect").addClass("btnUnSelect");
					}
				}
				...
				$tr.click(function(){
					...
					// 选中时复选框改变勾选与不勾选状态(新增以下10行)
					var Single = $tr.attr("single");
					if (Single && Single == "true"){$trs.each(function(){
							var $tr = $(this);
							$tr.find(":checkbox").attr('checked', false);
						});
						$trs.filter(".selected").find(":checkbox").attr('checked', true);
					}
					else {
						var $chk = $trs.filter(".selected").find(":checkbox");
