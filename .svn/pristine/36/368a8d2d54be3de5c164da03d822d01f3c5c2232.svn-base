...
(function($){
	$.pdialog = {
		...
		open:function(url, dlgid, title, options) {
			var op = $.extend({data:{}},$.pdialog._op, options);
...
//					$.pdialog.switchDialog(dialog);
		...
		attachShadow:function(dialog) {
			...
			shadow.css({
				top: parseInt($(dialog)[0].style.top) + 0,
				left: parseInt($(dialog)[0].style.left) - 8,
				height: parseInt($(dialog).height()) + 16,
				width: parseInt($(dialog).width()) + 16,
				zIndex:parseInt($(dialog).css("zIndex")) - 1
			});
			$(".shadow_c", shadow).children().andSelf().each(function(){
				$(this).css("height", $(dialog).outerHeight() - 13);
			});
		},
		...
		repaint:function(target,options){
			...
			if(target == "n" || target =="nw" || target == "ne") {
				shadow.css("top", options.otop + 0);
			}
			if(options.owidth && (target != "n" || target != "s")) {
				shadow.css("width", options.owidth + 16);
			}
			if(target.indexOf("w") >= 0) {
				shadow.css("left", options.oleft - 8);
			}
		},
		/**
		 * 改变左右拖动层的高度
