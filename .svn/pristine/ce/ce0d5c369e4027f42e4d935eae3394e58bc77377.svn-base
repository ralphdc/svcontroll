<?php
/**
 * 服务管服务和java通信公共数据接口类子类（密码管理）
 * @author zengguangqiu
 *
 */
class PassmanageModel extends CommunicationModel {
	var $server = '/omp/asset';
	var $method = array(
			'index'=>'asset.serverpwd.search',
			'toexcell'=>'asset.serverpwd.list',
	);
	 ...
	 * @return array
	 */
	function findByPostToExcel ($post, $get) {
		if(!empty($post)){
			foreach ($post as $key=>$valus){
				$post[$key]=trim($valus," ");
			}
